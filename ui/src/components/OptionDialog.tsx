...
export function OptionDialog({ choices, plan, onRespond }: OptionDialogProps) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/60">
      <div role="dialog" aria-modal="true" aria-labelledby={titleId} className="bg-gray-900 border border-gray-700 rounded-lg p-6 max-w-lg w-full space-y-4">
        <h2 id={titleId} className="text-gray-100 font-semibold text-lg">Choose an option</h2>
          ...
          {choices.map((choice, index) => {
            ...
            if (choice.kind === "textInput") {
              ...
              return (
                <div key={key} className="space-y-2">
                  <label htmlFor={inputId} className="text-sm text-gray-300 block">
                    {choice.label}
                  </label>
                  <input
                    id={inputId}
            ...
            return (
              <button
                ...
                className="w-full px-4 py-2 bg-gray-700 text-gray-200 rounded text-sm hover:bg-gray-600 text-left"
              >
                {choice.label}
              </button>
            );
