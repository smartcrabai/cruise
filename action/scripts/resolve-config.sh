...
#      default config and exports it as CRUISE_CONFIG. It deliberately sets
...
# default above, it sets neither `sdk` nor `model`/`plan_model`.
set -euo pipefail
...
# its own. Provides a minimal write-tests -> implement workflow on cruise's
# default backend, deliberately without `sdk` or `model`/`plan_model` so
# cruise picks `sdk: jcode` and jcode picks the provider/model configured in
# cruise's jcode home.
...
# referencing {input} rather than {plan}. `sdk` is omitted so cruise uses its
# own default backend, and `model` is omitted for the same reason as the
# generated default config above: it lets jcode resolve the model configured
# in cruise's jcode home instead of failing on a hardcoded literal.
