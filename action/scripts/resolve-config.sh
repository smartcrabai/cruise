...
#      no config is found at all) targets `sdk: seher` with seher mode keys
#      (`model: build` / `plan_model: plan`), which are meaningless under
#      the `sdk: pi` this action always forces. Leaving them unset lets pi
#      auto-select a provider/model from
...
# its own. Provides a minimal write-tests -> implement workflow under
# `sdk: pi`, deliberately without `model`/`plan_model` so pi auto-selects a
# provider/model.
sdk: pi
