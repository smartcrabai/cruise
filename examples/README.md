...
| [`cruise-openai-compatible.yml`](cruise-openai-compatible.yml) | Your models live behind OpenAI-compatible or Anthropic-compatible endpoints not already known to pi. Uses the concise `providers`/`provider_api_keys` inputs; use raw `pi_models_json` for advanced schemas. |
