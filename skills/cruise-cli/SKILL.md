---
name: cruise-cli
description: Use when running, operating, or troubleshooting the `cruise` CLI or its default interactive keyboard client — the YAML-driven coding-agent workflow orchestrator that wraps coding-agent CLIs. Covers command selection (plan / --plan / draft / run / exec / list / clean / config / ssh / webui), planning variants (--skip-planning / --grill / --formal-spec / --no-interactive-planning / --image / --repo), bounded `run --all --parallelism` execution, the live dashboard, the TUI's WebUI-domain session workflows, session lifecycle and phases, workspace modes, config resolution, runtime file layout. Trigger whenever the user asks how to start a cruise session, run or resume a workflow, manage or clean sessions, use the TUI, pick a workspace mode, or debug why a session is stuck, failed, or not picked up by `cruise run`.
---
...
- Bare `cruise` opens the official interactive keyboard client for the full WebUI-domain session workflows; use it for interactive management, while `cruise list` remains available as the CLI selector.
...
Ask and Option prompts are handled in the TUI. `ask_user` pauses its session without opening a modal: the session is marked **Awaiting Input**, and its question is shown only in that session's **Plan** tab. Press `o` or choose **Answer Prompt** on that session to open the Plan tab. Press `Enter` to edit, press `Enter` again to submit, and press `Esc` to leave editing while keeping the draft. Execution-time `Option` requests remain in the modal queue. A single-run Option opens automatically, while Run All shows a queue badge. Multiline `ask_user` questions are rendered as adjacent lines in the selected session's Plan tab. Its answer field is inline, session-scoped, and not a modal. PR and Issue URLs are shown as text, except that a successful Publish as Issue also automatically opens the published Issue URL. Dedicated PR/Issue URL actions open their URLs using `open` on macOS or `xdg-open` on Linux. Other Markdown links remain textual. The CLI-only `config` and `exec` operations remain CLI commands rather than TUI screens.

The New Session dialogue autosaves its answers 500 ms after a change. Other screen state is ephemeral, except that manually selected detail tabs are retained per session for the duration of the TUI process, including refreshes and planning updates. Only Delete, Discard, Reset to Planned, Publish as Issue, Clean, Run All / Cancel Run All, and quitting with active work ask for confirmation; Cancel, Approve, Run, Resume, Retry, Generate Plan, and Open PR execute immediately. In New Session, a non-blank task or at least one image attachment is required and the GitHub source requires a repository; a blank working directory means `.` and a blank workflow config means auto-detect. Cancelling a run moves its session to `Suspended`; cancelling planning restores the prior state and plan. Terminal state is restored on normal exit, panic, SIGTERM, and SIGHUP. Session errors stay in the app and session state; only terminal/root/event-loop failures exit the TUI.
...
These process environment variables are read directly at runtime and are **not** workflow-config overrides: `CRUISE_COMMIT_COAUTHOR_NAME` together with `CRUISE_COMMIT_COAUTHOR_EMAIL` (both required, trimmed) append a `Co-authored-by:` trailer to commits cruise creates; `CRUISE_TOOL_SOCKET` names the Unix socket a `cruise mcp-bridge` child dials, and is the default for its `--socket` flag; `CRUISE_DISABLE_NOTIFICATIONS=1` suppresses desktop notifications for the CLI and the TUI alike (only the exact value `1` disables them); `CRUISE_DISABLE_HERDR=1` (again only the exact value `1`) suppresses herdr reporting. When `HERDR_ENV=1`, `HERDR_PANE_ID`, and `HERDR_BIN_PATH` are all set, the TUI, `cruise plan`, `cruise run`, and `cruise exec` report `working` / `blocked` / `idle` pane state to herdr via `"$HERDR_BIN_PATH" pane report-agent` (source `custom:cruise`, agent `cruise`) and release that authority on exit; every report is best-effort and never affects exit status.
...
- **Retries:** Without an SDK fallback policy, HTTP 429 uses exponential backoff (2s → 60s) with `--rate-limit-retries` retries (default 5, i.e. up to 6 attempts); an SDK `retry:` block or workflow-level model array with fallback entries also makes 5xx/network failures and HTTP 4xx other than 429, 401, 403, 407, and 408 retryable and can switch to fallback models using the same `--rate-limit-retries` budget — such a 4xx switches straight to the next chain entry instead of being resent to the same model, and surfaces unchanged when no entry is left. Whether the model it left behind is put on the 30-minute cooldown that a 429/5xx/network skip applies depends on what the failure blames: a refused request (`client error`) leaves that model immediately usable, while an error naming the model as absent (`400 model_not_supported`, `404 model_not_found`, `unknown model`) is a `missing model` and does cool it down, so later turns and later steps stop selecting a model the provider does not have. 401/403/407 are auth or permission failures another model cannot fix, so they fail the step immediately; 408 counts as a network failure, so it switches to a usable fallback first like a 5xx (same-model resend only when no fallback is left, visible text was already streamed, or `--rate-limit-retries` is `0`) and does cool the model it left behind down. Rate-limit wording and 5xx are classified ahead of 4xx-looking numbers, so a 429 or 503 keeps its class even when stderr noise in the message contains something 4xx-shaped. Budgeted graph transitions — including ordinary sequential transitions — are bounded by `--max-retries`, resolved as: the explicit flag wins, else the workflow config's top-level `max_retries`, else 3.
