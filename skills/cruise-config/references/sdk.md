...
Under the `@cruise` GitHub Action this is done through action inputs instead — `PI_CODING_AGENT_DIR` is a reserved name there and an `env:` entry setting it is dropped with a warning. Use the `providers` + `provider_api_keys` inputs to have the action generate the `models.json` for you, or `pi_models_json` to supply one verbatim; see `docs/github-actions.md`.
