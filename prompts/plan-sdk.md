...
3. **Whenever a requirement is genuinely ambiguous and the answer changes the plan, call the `ask_user` tool to ask the user a focused question instead of guessing. Prefer asking over assuming.**
...
**When the plan is complete, submit it by calling the `submit_plan` tool with the full plan as markdown.** Do not write the plan to a file directly — `submit_plan` persists it. Submit exactly once, after any clarifying questions are resolved.
